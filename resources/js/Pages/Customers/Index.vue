        ...
        <div class="p-4 bg-white rounded-lg shadow-xs mt-6 space-y-6">
            <!-- <div class="inline-flex overflow-hidden mb-4 w-full bg-white rounded-lg shadow-md">
                ...
                <div class="px-4 py-2 -mx-3 ">
                    <div class="mx-3">
                        <Link class="ml-6 text-lg font-bold text-gray-800" :href="route('customers.create')">
                            ...
                            <tr v-for="(customer, index) in customers.data" :key="customer.id"
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <label class="relative inline-flex items-center mr-5 cursor-pointer">
                                        <input type="checkbox"  class="sr-only peer" :checked="customer.status === 1" @change="updateStatus(customer.id, index)">
    ...
    export default {
        ...
        methods: {
            updateStatus(id,index){
                axios.post(route('updateStatus', { id: id, model: 'Customer' }))
                .then(response => {
                    this.customers.data[index].status = response.data.status;
                });
            }
        },
    }
