        ...
        <table class="w-full whitespace-no-wrap">
            <thead>
                <tr
                    ...
                    <th class="px-2 py-3">Account No</th>
                    <th class="px-5 py-3">Account Name</th>
                    <th class="px-2 py-3">Last Paid Amount</th>
                    <th class="px-2 py-3">Payable Amount</th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y"  >
                <tr v-for="(vendorBalance, index) in vendorBalances" :key="index">
                    
            
                    <td class="px-1 py-2 text-sm">{{ index }}</td>

                    <td class="px-1 py-2 text-sm">{{ index }}</td>
                    <td class="px-4 py-2 text-sm">{{ vendorBalance.name }}</td>
                    <td class="px-1 py-2 text-sm">{{ vendorBalance.latestVoucherAmount }}</td>
                    <td class="px-1 py-2 text-sm">{{ vendorBalance.formattedBalance }}  </td>
...
<script>
    ...
    export default {
        ...
        props:{
            start_date: Date, end_date: Date, vendorBalances: Array
        },
